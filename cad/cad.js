...
const measuredButton=document.createElement('button');measuredButton.textContent='Edit measured / angled outline';$('correctoutline').after(measuredButton);
measuredButton.onclick=()=>run(async()=>{const corrected=await PanelMeasuredOutline.open(spec,panels[panelIndex]?.file);if(!corrected)return;if(panelIndex<0){addPanel(corrected,'Measured panel');}else{spec=corrected;panels[panelIndex].spec=spec;renderSpec();rememberPanel();}notice('Measured outline applied. Review the dimensions before generating.');});
...
$('import').onchange=()=>run(async()=>{const file=$('import').files[0];if(!file||file.size>128*1024)throw Error('Choose a panel draft smaller than 128 KB.');const data=JSON.parse(await file.text());if(data.measuredEdges?PanelMeasuredOutline.validate(data).length:(!Array.isArray(data.edges)||data.edges.length<4||data.edges.length>32||!data.edges.every(e=>e&&codes.includes(e.code)&&directions.includes(e.direction))))throw Error('Invalid panel draft.');addPanel(data,file.name);notice('Draft loaded. Review it before generating.');});
