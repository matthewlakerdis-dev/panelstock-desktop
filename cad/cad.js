...
async function correctOutline(file){const original=spec;const corrected=await PanelOutlineCorrection.open(file,original);if(!corrected)return;recalculateOutline(corrected);spec=corrected;const p=panels[panelIndex];if(p){p.error=null;p.file=file;p.spec=spec;p.result=null;p.reviewed=false;}else{addPanel(spec,file.name);}renderSpec();rememberPanel();notice('Corrected outline applied. Review dimensions, folds and edge types before generating.');}
