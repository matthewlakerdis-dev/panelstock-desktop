...
function selectPanel(index){if(index<0||index>=panels.length)return;rememberPanel();panelIndex=index;const p=panels[index];spec=p.spec||null;
 ...
 $('download').disabled=!result;if($('placedimensions'))$('placedimensions').hidden=!result?.dimensionEditor;notice(p.error||p.message||'Select Read sketches to read this file.');updateNavigator();}
...
function invalidate(){if($('placedimensions'))$('placedimensions').hidden=true;const errorBox=$('generation-error');if(errorBox)errorBox.hidden=true;version++;result=null;$('download').disabled=true;$('confirmed').checked=false;$('preview').hidden=true;$('validation').textContent='Generate a new preview after reviewing your changes.';}
