...
function renderQuestions(){
 ...
 $('questions').hidden=!issues.length&&!notes.length;
 for(const [title,items] of [['Current panel checks',issues],['Original sketch-reading notes (not updated by edits)',notes]]){
...
function renderSpec(){invalidate();$('panelid').value=spec.panelId||'';$('folds').value=(spec.folds||[]).join(', ');$('edges').replaceChildren(...spec.edges.map(edgeRow));renderQuestions();}
...
function collect(){if(!spec)throw Error('Load a sketch or start a panel first.');const values=$('folds').value.trim();return {...spec,panelId:$('panelid').value.trim(),folds:values?values.split(',').map(x=>Number(x.trim())):[],reviewed:$('confirmed').checked};}
...
for(const id of ['panelid','folds'])$(id).addEventListener('input',()=>{invalidate();renderQuestions();});
