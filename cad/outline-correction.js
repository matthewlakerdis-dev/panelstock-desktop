...
async function open(file,draft){
 ...
 let points=[],values=[],closed=false,drag=null;
 q('[data-arrow]').value=draft?.panelDirection||'none';q('[data-id]').value=draft?.panelId||'';q('[data-folds]').value=(draft?.siteFolds||[]).join(', ');
 const finish=value=>{URL.revokeObjectURL(url);dialog.close();dialog.remove();resolve(value);};
