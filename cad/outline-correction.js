...
async function open(file,draft,readMeasurements){
 ...
 function fields(){q('[data-edges]').replaceChildren();values.forEach((v,i)=>{
 const row=document.createElement('div');row.className='trace-edge';const label=document.createElement('div');label.textContent='Section '+(i+1)+': point '+(i+1)+' → '+((i+1)%values.length+1);
 const k=v.kind||PanelSketchComponents.kind(points[i],points[(i+1)%points.length]);v.kind=k;
 const shape=document.createElement('select');shape.setAttribute('aria-label','Section '+(i+1)+' shape');for(const key of ['horizontal','vertical','sloping'])shape.append(new Option(key[0].toUpperCase()+key.slice(1),key));shape.value=k;shape.onchange=()=>{v.kind=shape.value;fields();};label.append(shape);
 for(const [key,text] of k==='sloping'?[['width','Across (mm)'],['height','Rise / drop (mm)']]:[['site','Site (mm)']]){const wrap=document.createElement('label');wrap.textContent=text;const input=document.createElement('input');input.type='number';input.min=k==='sloping'?'0':'.001';input.max='10000';input.step='any';input.value=v[key]??'';input.placeholder='Written measurement';input.setAttribute('aria-label','Section '+(i+1)+' '+text);input.oninput=()=>v[key]=input.value===''?null:Number(input.value);wrap.append(input);label.append(wrap);}
 const type=document.createElement('select');type.setAttribute('aria-label','Edge '+(i+1)+' type');for(const code of ['','B','S','NT','RE','FE','CR'])type.append(new Option(code||'Select type',code));type.value=v.code||'';type.onchange=()=>v.code=type.value;row.append(label,type);q('[data-edges]').append(row);
 });}
 ...
 q('[data-reset]').onclick=()=>{points=[];values=[];markedFolds=[];rightAngles=[];reliefEnds=[];marking=false;pendingFold=null;selectedFold=null;closed=false;fields();draw();status('Click the bottom-left panel-face corner to begin.');};
 q('[data-finish]').onclick=()=>{try{directions(points,true);closed=true;values=points.map((p,i)=>values[i]||({site:null,code:''}));fields();draw();status('Outline traced. Confirm every edge measurement and type.');}catch(e){status(e.message);}};
