...
async function open(file,draft,readMeasurements){
 ...
 function measurements(){
  ...
  for(let i=0;i<count;i++){
   ...
   badge.append(make('rect',{x:bx-72,y:by-15,width:144,height:30,rx:5,fill:'#fff',stroke:'#23627c'}),make('text',{x:bx,y:by+5,'text-anchor':'middle','font-size':16,fill:'#14394a'},caption()));
   ...
   const tag=document.createElement('select');tag.className='trace-tag';tag.setAttribute('aria-label','Section tag');for(const code of ['','B','S','NT','RE','FE','CR'])tag.append(new Option(code||'Select tag',code));tag.value=v.code||'';tag.onchange=()=>{v.code=tag.value;badge.querySelector('text').textContent=caption();};box.append(tag);
