...
async function open(file,draft,readMeasurements){
 ...
 function chooseAngle(target){const fh=target.closest('[data-fold]'),eh=target.closest('[data-edge]');if(!fh&&!eh){status('Select a marked fold line and the outline section it meets.');return;}const item=fh?{fold:Number(fh.dataset.fold)}:{edge:Number(eh.dataset.edge)};
 ...
 q('[data-angle]').onclick=()=>{if(!closed){status('Finish the outline and mark its folds first.');return;}angleMode=!angleMode;angleFirst=null;marking=false;editingSection=null;draw();status(angleMode?'Select a fold line and the outline section it meets. Select the same pair again to remove the mark.':'Right-angle tool off.');};
 ...
 q('[data-reset]').onclick=()=>{editingSection=null;angleMode=false;angleFirst=null;points=[];values=[];markedFolds=[];rightAngles=[];reliefEnds=[];marking=false;pendingFold=null;selectedFold=null;closed=false;fields();draw();status('Click the bottom-left panel-face corner to begin.');};
