...
async function open(file,draft,readMeasurements){
 ...
 function draw(){q('[data-angle]').setAttribute('aria-pressed',String(angleMode));svg.setAttribute("tabindex","0");svg.replaceChildren(make('image',{href:url,x:0,y:0,width:1000,height:1000,preserveAspectRatio:'none'}));
  ...
  markedFolds.forEach((f,i)=>{if(!points[f.from]||!points[f.to])return;const a=points[f.from],b=points[f.to];const line=make('line',{x1:a.x,y1:a.y,x2:b.x,y2:b.y,stroke:selectedFold===i?'#a44912':'#bc7928','stroke-width':6,'stroke-dasharray':'12 7','data-fold':i,tabindex:0,role:'button','aria-label':'Fold '+(i+1)});svg.append(line);});
 ...
 function chooseAngle(target){const fh=target.closest('[data-fold]'),eh=target.closest('[data-edge]');if(!fh&&!eh){status('Select two intersecting folds, or a fold and its adjoining outline section.');return;}const item=fh?{fold:Number(fh.dataset.fold)}:{edge:Number(eh.dataset.edge)};
 ...
 q('[data-angle]').onclick=()=>{if(!closed){status('Finish the outline and mark its folds first.');return;}angleMode=!angleMode;angleFirst=null;marking=false;editingSection=null;draw();status(angleMode?'Select two intersecting folds, or a fold and its adjoining outline section. Select the same pair again to remove the mark.':'Right-angle tool off.');};
 ...
 q('[data-reset]').onclick=()=>{editingSection=null;angleMode=false;angleFirst=null;points=[];values=[];markedFolds=[];foldRightAngles=[];rightAngles=[];reliefEnds=[];marking=false;pendingFold=null;selectedFold=null;closed=false;fields();draw();status('Click the bottom-left panel-face corner to begin.');};
 ...
 if(angled()||rightAngles.length||reliefEnds.length){const solved=PanelSketchComponents.resolve(points,values,markedFolds,{rightAngles,reliefEnds});values=solved.values;const model=PanelSketchComponents.build(points,values,markedFolds,{rightAngles,reliefEnds});checkFoldAngles(model.measuredFolds,foldRightAngles);const allErrors=PanelMeasuredOutline.validate(model),pendingFolds=allErrors.some(e=>e==='Measured diagonal outlines currently support horizontal internal folds.');const errors=allErrors.filter(e=>e!=='Measured diagonal outlines currently support horizontal internal folds.');if(errors.length)throw Error(errors.join(' '));finish({...draft,...model,panelId:q('[data-id]').value.trim(),panelDirection:q('[data-arrow]').value,outlineSections:points.map((start,i)=>({start:{...start},...values[i]})),markedFolds,foldRightAngles,reviewed:false,unsupported:pendingFolds,questions:['Review the written measurements and marked folds before generating.',...solved.notes,...(pendingFolds?['Fold marks saved; machining mixed or angled fold orientations is not yet supported.']:[])],directionSource:'manual-sketch-trace'});return;}
