...
async function open(file,draft,readMeasurements){
 ...
 function measurements(){
  ...
  for(let i=0;i<count;i++){
   ...
   badge.append(make('rect',{x:-25,y:-13,width:50,height:26,fill:'transparent',stroke:'none',class:'trace-dimension-gap'}),make('text',{x:bx,y:by,'text-anchor':'middle','dominant-baseline':'central','font-size':20,fill:'#344954'},caption()));
   ...
   const placeCard=()=>{const m=svg.getScreenCTM();if(!m)return;const sx=Math.abs(m.a)||1,sy=Math.abs(m.d)||1,vb=svg.viewBox.baseVal;const cx=Math.max(vb.x+5/sx,Math.min(vb.x+vb.width-(cw+5)/sx,x-cw/(2*sx))),cy=y+(ch+24)/sy<vb.y+vb.height?y+24/sy:Math.max(vb.y+5/sy,y-(ch+24)/sy);card.setAttribute('transform','translate('+cx+' '+cy+') scale('+(1/sx)+' '+(1/sy)+')');};placeCard();requestAnimationFrame(()=>{if(card.isConnected)placeCard();});
