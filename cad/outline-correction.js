...
async function open(file,draft,readMeasurements){
 ...
 function measurements(){
  ...
  for(let i=0;i<count;i++){
   ...
   const caption=()=> (k==='sloping'?(v.width??'?')+' × '+(v.height??'?'):(v.site??'Add mm'))+(v.site!=null||k==='sloping'?' mm':'')+(v.code?' · '+v.code:'');
   const badge=make('g',{'data-measurement':i,tabindex:0,role:'button','aria-label':'Section '+(i+1)+' measurement',class:'trace-dimension'});
   const bx=Math.max(85,Math.min(915,x)),by=Math.max(22,Math.min(978,y-22));
   badge.append(make('rect',{x:bx-82,y:by-18,width:164,height:36,rx:8,fill:'#fff',stroke:'#23627c'}),make('text',{x:bx,y:by+6,'text-anchor':'middle','font-size':18,fill:'#14394a'},caption()));
   badge.onclick=()=>{if(!reading)editSection(i);};badge.onkeydown=e=>{if(e.key==='Enter'||e.key===' '){e.preventDefault();if(!reading)editSection(i);}};svg.append(badge);
   if(editingSection!==i)continue;
   const card=make('foreignObject',{x:Math.max(5,Math.min(675,x-150)),y:Math.max(5,Math.min(640,y+12)),width:320,height:355,'data-inline-editor':i});
   const box=document.createElement('div');box.className='trace-inline-editor';const title=document.createElement('strong');title.textContent='Section '+(i+1);box.append(title);
   const shape=document.createElement('select');shape.setAttribute('aria-label','Section shape');for(const key of ['horizontal','vertical','sloping'])shape.append(new Option(key[0].toUpperCase()+key.slice(1),key));shape.value=k;shape.onchange=()=>{v.kind=shape.value;draw();};box.append(shape);
   for(const [key,text]of k==='sloping'?[['width','Across (mm)'],['height','Rise / drop (mm)']]:[['site','Written measurement (mm)']]){const label=document.createElement('label');label.textContent=text;const input=document.createElement('input');input.type='number';input.inputMode='decimal';input.step='any';input.min=k==='sloping'?'0':'.001';input.max='10000';input.value=v[key]??'';input.oninput=()=>{v[key]=input.value===''?null:Number(input.value);badge.querySelector('text').textContent=caption();};label.append(input);box.append(label);}
   const tag=document.createElement('select');tag.setAttribute('aria-label','Section tag');for(const code of ['','B','S','NT','RE','FE','CR'])tag.append(new Option(code||'Select tag',code));tag.value=v.code||'';tag.onchange=()=>{v.code=tag.value;badge.querySelector('text').textContent=caption();};box.append(tag);
   const done=document.createElement('button');done.type='button';done.textContent='Done';done.onclick=()=>{editingSection=null;draw();svg.focus();};box.append(done);box.onkeydown=e=>{if(e.key==='Enter'&&e.target.tagName!=='SELECT'){e.preventDefault();e.stopPropagation();done.click();}};
 ...
 if(angled()||rightAngles.length||reliefEnds.length){const model=PanelSketchComponents.build(points,values,markedFolds,{rightAngles,reliefEnds});const errors=PanelMeasuredOutline.validate(model);if(errors.length)throw Error(errors.join(' '));finish({...draft,...model,panelId:q('[data-id]').value.trim(),panelDirection:q('[data-arrow]').value,outlineSections:points.map((start,i)=>({start:{...start},...values[i]})),markedFolds,reviewed:false,unsupported:false,questions:['Review the written measurements and marked folds before generating.'],directionSource:'manual-sketch-trace'});return;}
