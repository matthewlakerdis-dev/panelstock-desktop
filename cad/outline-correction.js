...
async function open(file,draft,readMeasurements){
 ...
 dialog.innerHTML='<header><div><h2>Correct the sketch outline</h2><p>Start bottom-left towards the right. Click every corner and each horizontal fold where it meets a side. Enter the measurement between consecutive points. Ignore the outer tag flaps.</p></div><button type="button" data-close aria-label="Close outline correction">×</button></header><div class="trace-layout"><div><svg viewBox="0 0 1000 1000" preserveAspectRatio="none" aria-label="Sketch with editable outline"></svg><div class="trace-tools"><button type="button" data-undo>Undo last corner</button><button type="button" data-reset>Trace again</button><button type="button" data-finish>Finish outline</button><button type="button" data-mark aria-pressed="false">Mark folds</button><button type="button" data-delete-fold>Delete selected fold</button><button type="button" data-read>Read measurements from sketch</button></div></div><div class="trace-values"><p>Enter the written fold-to-fold measurements. Side sections are added together and fold heights are calculated automatically.</p><label>Panel ID<input data-id maxlength="60"></label><label>Site fold heights from bottom (mm)<input data-folds placeholder="e.g. 868"></label><label>Panel arrow direction<select data-arrow><option value="none">No arrow</option><option value="right">Right</option><option value="left">Left</option><option value="up">Up</option><option value="down">Down</option></select></label><p>Add outline points at fold endpoints, then choose Mark folds and click two numbered points. Select a fold to replace its endpoints or delete it.</p><div data-marked-folds></div><div data-edges></div></div></div><p role="status" data-status></p><footer><button type="button" data-cancel>Cancel</button><button type="button" class="primary" data-apply>Apply corrected outline</button></footer>';
